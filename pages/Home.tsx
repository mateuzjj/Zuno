        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.recent.map((track) => (
            <div
              ...
              className="flex items-center gap-4 bg-zuno-card/50 hover:bg-zuno-card transition-all rounded-card overflow-hidden cursor-pointer group pr-4 border border-white/5 hover:border-white/10"
            >
              ...
              <div className="flex-1 min-w-0 py-2">
                <h3 className="font-bold text-white truncate text-sm md:text-base group-hover:text-zuno-accent transition-colors">{track.title}</h3>
                <p className="text-xs text-zuno-muted truncate">{track.artist}</p>
              </div>
              <div className="opacity-0 group-hover:opacity-100 bg-zuno-accent rounded-full p-3 shadow-lg transition-all transform scale-90 group-hover:scale-100 translate-y-2 group-hover:translate-y-0 text-black">
